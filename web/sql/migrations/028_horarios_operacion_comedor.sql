...
END;
IF OBJECT_ID(N'dbo.Horario', N'U') IS NOT NULL AND COL_LENGTH(N'dbo.Horario', N'HoraLimite') IS NOT NULL AND COL_LENGTH(N'dbo.Horario', N'IdHorario') IS NOT NULL AND COL_LENGTH(N'dbo.Horario', N'Descripcion') IS NOT NULL
BEGIN
    ...
    EXEC sys.sp_executesql N'
        ...
        IF EXISTS (SELECT codigo FROM (SELECT CASE WHEN UPPER(LTRIM(RTRIM(Descripcion))) LIKE N''%NOCTURN%'' THEN ''nocturno'' ELSE ''diurno'' END codigo FROM dbo.Horario) clasificados GROUP BY codigo HAVING COUNT(*)>1)
        ...
        SELECT CASE WHEN UPPER(LTRIM(RTRIM(Descripcion))) LIKE N''%NOCTURN%'' THEN ''nocturno'' ELSE ''diurno'' END,
               Descripcion,HoraLimite,''dbo.Horario'',HoraLimite,IdHorario
        FROM dbo.Horario h
...
IF OBJECT_ID(N'dbo.Horario', N'U') IS NOT NULL AND COL_LENGTH(N'dbo.Horario', N'Descripcion') IS NULL
