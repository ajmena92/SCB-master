...
function abrirReportePines(credenciales: CredencialTemporal[], reporte: ReportePines) {
  ...
  ventana.document.write(`<!doctype html><html lang="es"><head><title>PIN ${reporte.seccion} ${reporte.anio}</title><style>body{font-family:"Segoe UI",sans-serif;color:#111827;margin:28px}header{border-bottom:2px solid #0a84ff;padding-bottom:14px;margin-bottom:18px}h1,p{margin:0}h1{font-size:21px}small{color:#475569}table{width:100%;border-collapse:collapse;margin-top:18px}th,td{padding:9px;border-bottom:1px solid #cbd5e1;text-align:left}th{background:#edf5ff;font-size:12px;text-transform:uppercase}.pin{font-size:18px;font-weight:700;letter-spacing:2px}footer{margin-top:22px;padding-top:10px;border-top:1px solid #cbd5e1;font-size:11px;color:#475569}@media print{body{margin:14mm}}</style></head><body><header><p>CTP Platanares · Entrega segura de credenciales</p><h1>PIN temporales · Sección ${escaparHtml(reporte.seccion)} · ${reporte.anio}</h1><small>Generado ${escaparHtml(fecha)} · ${credenciales.length} estudiantes activos</small></header><table><thead><tr><th>#</th><th>Estudiante</th><th>Cédula</th><th>PIN temporal</th></tr></thead><tbody>${filas}</tbody></table><footer>Documento confidencial. Entregar individualmente y destruir cuando los PIN hayan sido cambiados.</footer></body></html>`);
