...
export default function EditarEstudiante() {
  ...
  if (!referencia || !persona || persona.tipo !== "estudiante")
    return (
      <section className="grid gap-4">
        <Aviso tipo="error">
          No se encontró el estudiante solicitado. Vuelva al padrón y selecciónelo de nuevo.
        </Aviso>
        <button
  ...
  return (
    <section className="mx-auto grid max-w-6xl gap-4 pb-8">
      ...
      <AlertDialog
        ...
        onOpenChange={(abierto) => !abierto && setConfirmacion(undefined)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={ocupada}
              onClick={() => (confirmacion === "pin" ? reiniciarPin.mutate() : desactivar.mutate())}
            >
