...
export default function ParametrosOperativos() {
  ...
  const horarios = useQuery({
    ...
    queryFn: plataformaApi.tiquetes.horariosReserva,
  });
  ...
  const error =
    ...
    institucion.error ||
    crear.error ||
    guardarHorario.error ||
    guardarInstitucion.error;
  function nuevaTarifa(evento: FormEvent<HTMLFormElement>) {
    evento.preventDefault();
    const datos = new FormData(evento.currentTarget);
    crear.mutate({
      tipoPersona: datos.get("tipoPersona") === "profesor" ? "profesor" : "estudiante",
      montoColones: Number(datos.get("monto")),
      vigenteDesde: String(datos.get("desde")),
      vigenteHasta: null,
    });
  }
  return (
    <section className="stack">
      <EncabezadoPagina
        titulo="Parámetros operativos"
        descripcion="Configure tarifas de tiquetes y la hora límite de marca de comedor."
      />
      ...
      <section className="action-panel">
        <div className="panel-title">
          ...
          <div>
            <h2>Tarifas de tiquetes</h2>
            <p>La venta aplica automáticamente la tarifa vigente según el tipo de persona.</p>
          </div>
        </div>
        <form className="form-grid" onSubmit={nuevaTarifa}>
          ...
          <Campo etiqueta="Precio por tiquete (₡)">
            <input name="monto" type="number" min="0" required />
          </Campo>
          <Campo etiqueta="Vigente desde">
            <input name="desde" type="date" required />
          </Campo>
          <button className="button primary" disabled={crear.isPending}>
            {crear.isPending ? "Guardando…" : "Programar tarifa"}
          </button>
        </form>
        {tarifas.isLoading ? (
          <EstadoCarga />
          ...
          <Tabla
            ...
            filas={(tarifas.data?.elementos ?? []).map((tarifa) => [
              tarifa.tipoPersona,
              `₡${tarifa.montoColones.toLocaleString("es-CR")}`,
              tarifa.vigenteDesde,
              tarifa.vigenteHasta ?? "Vigente",
            ])}
          />
        )}
      </section>
      <section className="action-panel">
        <div className="panel-title">
          ...
          <div>
            <h2>Hora límite de marca de comedor</h2>
            <p>Después de esta hora, la reserva del horario correspondiente queda cerrada.</p>
          </div>
        </div>
        {horarios.isLoading ? (
          <EstadoCarga />
          ...
          <div className="settings-hours">
            {horarios.data?.map((horario) => (
              <form
                ...
                onSubmit={(evento) => {
                  evento.preventDefault();
                  guardarHorario.mutate({
                    turno: horario.turno,
                    horaLimite: String(new FormData(evento.currentTarget).get("hora")),
                  });
                }}
              >
      ...
      <section className="action-panel">
        <div className="panel-title">
          ...
          <div>
            <h2>Identidad institucional en reportes</h2>
            <p>Estos datos aparecen en comprobantes y reportes generados por el sistema.</p>
          </div>
        </div>
        {institucion.isLoading ? (
          <EstadoCarga />
        ) : (
          <form
            className="form-grid"
            onSubmit={(evento) => {
              evento.preventDefault();
              const datos = new FormData(evento.currentTarget);
              guardarInstitucion.mutate({
                nombreColegio: String(datos.get("nombreColegio")),
                subtituloReportes: String(datos.get("subtituloReportes")),
              });
            }}
          >
            <Campo etiqueta="Nombre del colegio">
              <input
                name="nombreColegio"
                defaultValue={institucion.data?.nombreColegio}
                required
                maxLength={180}
              />
            </Campo>
            <Campo etiqueta="Subtítulo de reportes">
              <input
                name="subtituloReportes"
                defaultValue={institucion.data?.subtituloReportes}
                required
                maxLength={220}
              />
            </Campo>
            <button className="button primary" disabled={guardarInstitucion.isPending}>
              {guardarInstitucion.isPending ? "Guardando…" : "Guardar identidad"}
            </button>
          </form>
        )}
      </section>
