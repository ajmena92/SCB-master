...
export default function PersonasMatriculas() {
  ...
  useEffect(() => {
    ...
    setAnioLectivoIdPines(
      anios.data.elementos.find((anio) => anio.vigente)?.id ?? anios.data.elementos[0].id,
    );
  ...
  return (
    <section>
      ...
      />
      <AlertDialog
        open={Boolean(confirmacionPin)}
        onOpenChange={(abierto) => !abierto && !reiniciandoPin && setConfirmacionPin(undefined)}
      >
        <AlertDialogContent aria-busy={reiniciandoPin}>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {reiniciandoPin ? "Generando credenciales" : "¿Reiniciar el PIN?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {reiniciandoPin
          ...
          {reiniciandoPin ? (
            ...
            <AlertDialogFooter>
              ...
              <button className="button primary" type="button" onClick={confirmarReinicioPin}>
                Reiniciar PIN
              </button>
            </AlertDialogFooter>
          )}
      ...
      <Dialog open={operacionesPinAbiertas} onOpenChange={setOperacionesPinAbiertas}>
        <DialogContent className="pin-operations-dialog sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Genere nuevas credenciales temporales y prepare el reporte para entregar los PIN de
              una sección.
            </DialogDescription>
          </DialogHeader>
          <DialogBody>
            <form id="operaciones-pin" className="pin-operations-form" onSubmit={enviarGrupo}>
              <Campo etiqueta="Año lectivo">
                <select
                  ...
                  required
                  value={anioLectivoIdPines ?? ""}
                  onChange={(evento) => {
                    setAnioLectivoIdPines(Number(evento.target.value) || undefined);
                    setSeccionPines("");
                  }}
                >
                  {anios.data?.elementos.map((anio) => (
              ...
              <Campo etiqueta="Sección">
                <select
                  ...
                  required
                  value={seccionPines}
                  disabled={!anioLectivoIdPines || secciones.isLoading}
                  onChange={(evento) => setSeccionPines(evento.target.value)}
                >
                  <option value="">
                    {secciones.isLoading ? "Cargando secciones…" : "Seleccione una sección"}
                  </option>
                  {secciones.data?.elementos.map((seccion) => (
              ...
              <div className="pin-section-summary" aria-live="polite">
                ...
                <span>
                  {seccionPines
                    ? resumenPines.isLoading
                      ? "Verificando sección…"
                      : `${resumenPines.data?.estudiantesActivos ?? 0} estudiantes activos recibirán un PIN nuevo`
          ...
          <DialogFooter>
            <button
              ...
              type="button"
              onClick={() => setOperacionesPinAbiertas(false)}
            >
              Cancelar
            </button>
            <button
              ...
              disabled={
                reiniciarGrupo.isPending || !seccionPines || !resumenPines.data?.estudiantesActivos
              }
            >
              Continuar
            </button>
