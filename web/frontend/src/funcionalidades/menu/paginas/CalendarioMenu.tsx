...
export default function CalendarioMenu() {
  ...
  const calendario = useQuery({
    ...
    queryFn: async () =>
      (await api.get<DiaCalendario[]>("/v1/menu/calendario", { params: rango })).data,
  });
  const actualizar = useMutation({
    mutationFn: (dia: Pick<DiaCalendario, "fecha" | "habilitado">) =>
      api.put("/v1/menu/calendario", dia),
    onSuccess: () => {
      setDetalle(null);
  ...
  const guardarSustitucion = useMutation({
    mutationFn: (datos: NonNullable<typeof sustitucion>) =>
      api.put(`/v1/menu/sustituciones/${datos.fecha}`, {
        fecha: datos.fecha,
        titulo: datos.titulo,
  ...
  return (
    <section className="space-y-6" aria-labelledby="calendario-menu-titulo">
      ...
      {calendario.error ? (
        <p role="alert" className="rounded-xl border border-destructive/30 p-4 text-destructive">
          {errMsg(calendario.error)}
        </p>
      ) : null}
                ...
                {semana.map((dia, posicion) => {
                  ...
                  return (
                    <article
                      ...
                      onClick={() => dia.esLectivo && setDetalle(dia)}
                      onKeyDown={(evento) => {
          ...
          <DialogFooter className="gap-2 sm:gap-0">
              ...
              <Button
                ...
                onClick={() => {
                  setSustitucion({
                    ...
                    observaciones: "",
                    componentes: detalle.componentes.map(componenteSustitucion),
                  });
                  setDetalle(null);
                }}
              >
            ...
            {detalle && (
              <Button
                ...
                onClick={() =>
                  actualizar.mutate({ fecha: detalle.fecha, habilitado: !detalle.habilitado })
                }
              >
      ...
      <Dialog
        open={sustitucion !== null}
        onOpenChange={(abierto) => !abierto && setSustitucion(null)}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setSustitucion({ ...sustitucion, titulo: e.target.value })}
                />
              </div>
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setSustitucion({ ...sustitucion, observaciones: e.target.value })
                  }
                />
              </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setSustitucion(null)}>
              Cancelar
            </Button>
            <Button
              disabled={
                ...
                guardarSustitucion.isPending
              }
              onClick={() => sustitucion && guardarSustitucion.mutate(sustitucion)}
            >
              Guardar sustitución
            </Button>
          </DialogFooter>
