...
export function useMarcacionComedor() {
  ...
  const [pequeno, setPequeno] = useState(() => window.innerWidth < 1280 || window.innerHeight < 720);
  const [ultimoIngreso, setUltimoIngreso] = useState<IngresoSalida | null>(null);
  const [errorOperacion, setErrorOperacion] = useState<{ codigo: string; mensaje: string } | null>(null);
  ...
  async function registrar() {
    ...
    try {
      ...
      window.setTimeout(() => setUltimoIngreso(null), 60_000);
