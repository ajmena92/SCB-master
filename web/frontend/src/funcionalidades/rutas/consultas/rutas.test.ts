...
describe("consultas de rutas", () => {
  it("normaliza, valida y ejecuta operaciones canónicas", async () => {
    ...
    expect(validarRuta({ codigo: "", descripcion: "x" })).toContain("obligatorio");
    expect(validarRuta({ codigo: "0", descripcion: "Norte" })).toContain("protegida");
    expect(validarRuta({ codigo: "A", descripcion: "corta" })).toContain("más de 5");
    expect(validarRuta({ codigo: "A", descripcion: "Ruta Norte" })).toBe("");
    vi.spyOn(api, "get")
      .mockResolvedValueOnce({ data: [] } as never)
      .mockResolvedValueOnce({ data: [] } as never);
    vi.spyOn(api, "post").mockResolvedValue({} as never);
    vi.spyOn(api, "put").mockResolvedValue({} as never);
    await obtenerDatosRutas();
    await crearRuta({ codigo: "A", descripcion: "Ruta Norte", colorHex: "#fff", activo: true });
    await actualizarRuta(2, {
      codigo: "A",
