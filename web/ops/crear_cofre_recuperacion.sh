...
uso() {
    printf 'Uso: %s --clave-publica CUSTODIO.asc [--clave-publica CUSTODIO2.asc]\n' "$0" >&2
    exit 64
}
...
mapfile -t destinatarios < <(
    gpg --homedir "$gnupg" --with-colons --list-keys |
        awk -F: '$1 == "pub" { esperar_huella=1; next } esperar_huella && $1 == "fpr" { print $10; esperar_huella=0 }'
)
(( ${#destinatarios[@]} == ${#publicas[@]} )) || {
    printf 'ERROR: Cada archivo debe contener exactamente una identidad pública.\n' >&2; exit 1;
}
...
(cd "$publicar" && sha256sum "$(basename "$salida")" >"$(basename "$salida").sha256")
