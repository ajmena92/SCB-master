...
- `compose.local.yml` declara sus builds para API, frontend y migración. El
  entorno local sigue construyendo desde el código y conserva su volumen externo.
- `compose.prod-deploy.yml` sustituye esos builds por imágenes de API, frontend,
  migración y trabajador de importación identificadas por digest.
- `deploy-production.sh --remote` valida los cuatro digests, hace `pull` y usa
...
3. Registrar los cuatro digests aprobados en el archivo de entorno protegido del
...
- Las acciones de GitHub aún usan etiquetas de versión; fijarlas a SHA es una
  mejora posterior controlada, no se inventan SHA sin verificarlos.
