...
| --- | --- | --- | --- |
| ADR de un único modelo de sesión | Responsable técnico: ajmena92; revisión: Codex + validación de ajmena92 | 2026-09-15 | **Aceptada**: [ADR-0003](decisiones/0003-sesion-cookie-y-csrf.md) adopta cookie `HttpOnly`/`Secure`/`SameSite=Lax`, sin `Domain` amplio, y CSRF firmado o vinculado criptográficamente a sesión. Fase 1 debe validar `Origin`, CORS con `PATCH`/`X-CSRF-Token`, respuestas `401`/`403` sin mutación, login/logout/renovación y retirar tokens de las respuestas de login. También debe migrar `CARNET_QR_CLAVE` a secreto montado, sin rotarla. |
| Nombrar responsable, revisor y fecha de cada fase | Responsable técnico: ajmena92; DBA: administrador de producción | 2026-09-15 | **Asignado**: Codex apoya la revisión frontend y ajmena92 valida los cierres; el DBA administra cambios de PostgreSQL. |
