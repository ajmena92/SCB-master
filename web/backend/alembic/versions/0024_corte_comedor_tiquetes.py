"""Corta comedor al catálogo de personas y tiquetes con reservas atómicas."""
...
revision: str = "0024_corte_comedor_tiquetes"
down_revision: Union[str, None] = "0023_registro_comedor_modalidad"
        ...
        sa.text(
            ...
            BEGIN
                CREATE TABLE comedor.persona (
                    id_persona INT IDENTITY(1,1) NOT NULL
                        CONSTRAINT PK_comedor_persona PRIMARY KEY,
                    ...
                    creado_en DATETIME2(3) NOT NULL
                        CONSTRAINT DF_comedor_persona_creado DEFAULT SYSUTCDATETIME(),
                    actualizado_en DATETIME2(3) NOT NULL
                        CONSTRAINT DF_comedor_persona_actualizado DEFAULT SYSUTCDATETIME(),
                    CONSTRAINT CK_comedor_persona_tipo
                        CHECK (tipo_persona IN ('estudiante', 'profesor')),
                    CONSTRAINT CK_comedor_persona_estado
                        CHECK (estado_comedor IN ('becado_comedor', 'no_becado_comedor')),
                    CONSTRAINT CK_comedor_persona_vinculo
                        CHECK ((tipo_persona='estudiante' AND id_estudiante IS NOT NULL AND id_usuario IS NULL)
                            OR (tipo_persona='profesor' AND id_estudiante IS NULL AND id_usuario IS NOT NULL))
                );
            END;
            IF EXISTS (
                SELECT codigo_barras FROM comedor.persona
                GROUP BY codigo_barras HAVING COUNT(*) > 1
            )
            ...
            IF OBJECT_ID(N'estudiantes.estudiante', N'U') IS NOT NULL
               AND EXISTS (
                    ...
                    WHERE e.carne IS NOT NULL AND NULLIF(LTRIM(RTRIM(e.carne)), '') IS NOT NULL
            ...
            IF NOT EXISTS (SELECT 1 FROM sys.indexes
                WHERE object_id=OBJECT_ID(N'comedor.persona') AND name=N'UQ_comedor_persona_barcode')
                CREATE UNIQUE INDEX UQ_comedor_persona_barcode ON comedor.persona(codigo_barras);
            IF NOT EXISTS (SELECT 1 FROM sys.indexes
                WHERE object_id=OBJECT_ID(N'comedor.persona') AND name=N'UQ_comedor_persona_estudiante')
                CREATE UNIQUE INDEX UQ_comedor_persona_estudiante
                    ON comedor.persona(id_estudiante) WHERE id_estudiante IS NOT NULL;
            IF NOT EXISTS (SELECT 1 FROM sys.indexes
                WHERE object_id=OBJECT_ID(N'comedor.persona') AND name=N'UQ_comedor_persona_usuario')
                CREATE UNIQUE INDEX UQ_comedor_persona_usuario
                    ON comedor.persona(id_usuario) WHERE id_usuario IS NOT NULL;
            IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_comedor_persona_estudiante')
                ALTER TABLE comedor.persona ADD CONSTRAINT FK_comedor_persona_estudiante
                    FOREIGN KEY(id_estudiante) REFERENCES estudiantes.estudiante(id_estudiante);
            IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_comedor_persona_usuario')
                ALTER TABLE comedor.persona ADD CONSTRAINT FK_comedor_persona_usuario
                    FOREIGN KEY(id_usuario) REFERENCES identidad.usuario(id_usuario);
            IF EXISTS (
                SELECT 1 FROM comedor.persona p
                LEFT JOIN estudiantes.estudiante e ON e.id_estudiante=p.id_estudiante
                WHERE p.tipo_persona='estudiante' AND e.id_estudiante IS NULL
            )
                THROW 50029, 'Corte comedor abortado: existen personas estudiante huérfanas', 1;
            IF EXISTS (
                SELECT 1 FROM comedor.persona p
                LEFT JOIN identidad.usuario u ON u.id_usuario=p.id_usuario
                WHERE p.tipo_persona='profesor' AND u.id_usuario IS NULL
            )
                THROW 50030, 'Corte comedor abortado: existen personas profesor huérfanas', 1;
            """
        )
    )
    conexion.execute(
        sa.text(
            ...
            INSERT INTO comedor.persona
                (tipo_persona,id_estudiante,codigo_barras,nombre_completo,estado_comedor,activo)
            ...
            WHERE NOT EXISTS (
                SELECT 1 FROM comedor.persona p WHERE p.id_estudiante=e.id_estudiante
            );
            IF OBJECT_ID(N'comedor.cuenta_tiquetes', N'U') IS NULL
            BEGIN
                CREATE TABLE comedor.cuenta_tiquetes (
                    id_cuenta INT IDENTITY(1,1) NOT NULL
                        CONSTRAINT PK_comedor_cuenta_tiquetes PRIMARY KEY,
                    ...
                    actualizado_en DATETIME2(3) NOT NULL
                        CONSTRAINT DF_comedor_cuenta_actualizado DEFAULT SYSUTCDATETIME(),
                    CONSTRAINT UQ_comedor_cuenta_persona UNIQUE(id_persona),
                    ...
                    CONSTRAINT FK_comedor_cuenta_persona FOREIGN KEY(id_persona)
                        REFERENCES comedor.persona(id_persona)
                );
            END;
            """
        )
    ...
    conexion.execute(
        sa.text(
            ...
            BEGIN
                CREATE TABLE comedor.movimiento_tiquetes (
                    id_movimiento BIGINT IDENTITY(1,1) NOT NULL
                        CONSTRAINT PK_comedor_movimiento_tiquetes PRIMARY KEY,
                    ...
                    creado_por INT NULL,
                    creado_en DATETIME2(3) NOT NULL
                        CONSTRAINT DF_comedor_movimiento_creado DEFAULT SYSUTCDATETIME(),
                    CONSTRAINT CK_comedor_movimiento_tipo
                        CHECK (tipo IN ('recarga','consumo','reserva','liberacion','ajuste')),
                    ...
                    CONSTRAINT FK_comedor_movimiento_cuenta FOREIGN KEY(id_cuenta)
                        REFERENCES comedor.cuenta_tiquetes(id_cuenta)
                );
            END;
            ...
            BEGIN
                CREATE TABLE comedor.reserva (
                    id_reserva BIGINT IDENTITY(1,1) NOT NULL
                        CONSTRAINT PK_comedor_reserva PRIMARY KEY,
                    id_persona INT NOT NULL,
                    fecha DATE NOT NULL,
                    ...
                    creado_en DATETIME2(3) NOT NULL
                        CONSTRAINT DF_comedor_reserva_creado DEFAULT SYSUTCDATETIME(),
                    actualizada_en DATETIME2(3) NOT NULL
                        CONSTRAINT DF_comedor_reserva_actualizada DEFAULT SYSUTCDATETIME(),
                    CONSTRAINT UQ_comedor_reserva_persona_fecha UNIQUE(id_persona,fecha),
                    CONSTRAINT CK_comedor_reserva_estado
                        CHECK (estado IN ('reservada','cancelada','consumida')),
                    CONSTRAINT CK_comedor_reserva_modalidad
                        CHECK (modalidad IN ('beca','tiquete')),
                    CONSTRAINT FK_comedor_reserva_persona FOREIGN KEY(id_persona)
                        REFERENCES comedor.persona(id_persona)
                );
            END;
            ...
            BEGIN
                CREATE TABLE comedor.ingreso (
                    id_ingreso BIGINT IDENTITY(1,1) NOT NULL
                        CONSTRAINT PK_comedor_ingreso PRIMARY KEY,
                    id_persona INT NOT NULL,
                    fecha DATE NOT NULL,
                    modalidad VARCHAR(12) NOT NULL,
                    registrado_por INT NULL,
                    creado_en DATETIME2(3) NOT NULL
                        CONSTRAINT DF_comedor_ingreso_creado DEFAULT SYSUTCDATETIME(),
                    ...
                    CONSTRAINT FK_comedor_ingreso_persona FOREIGN KEY(id_persona)
                        REFERENCES comedor.persona(id_persona)
                );
            END;
            ...
            BEGIN
                ...
                WHERE NOT EXISTS (
                    SELECT 1 FROM comedor.registro_migracion_0024 a
                    WHERE a.id_registro=r.id_registro
                );
                IF EXISTS (
                ...
                IF EXISTS (
                    SELECT 1 FROM comedor.registro
                    WHERE modalidad NOT IN ('beca','tiquete')
                )
            ...
            WHERE (p.activo=1 OR EXISTS (
                SELECT 1 FROM comedor.ingreso i WHERE i.id_persona=p.id_persona
            ) OR EXISTS (
                SELECT 1 FROM comedor.reserva r WHERE r.id_persona=p.id_persona
            ))
            AND p.estado_comedor='no_becado_comedor'
            AND NOT EXISTS (
                SELECT 1 FROM comedor.cuenta_tiquetes c WHERE c.id_persona=p.id_persona
            );
            """
        )
    )
...
def downgrade() -> None:
    raise RuntimeError("El corte de comedor no admite reversión destructiva")
