"""Crea el catálogo web de horarios y límites del comedor."""
...
revision: str = "0028_horarios_operacion_comedor"
down_revision: Union[str, None] = "0027_catalogo_profesores"
    ...
    conexion.execute(
        sa.text(
            ...
            BEGIN
                CREATE TABLE comedor.horario_operacion (
                    id_horario INT IDENTITY(1,1) NOT NULL
                        CONSTRAINT PK_comedor_horario_operacion PRIMARY KEY,
                    ...
                    actualizado_en DATETIME2(3) NOT NULL
                        CONSTRAINT DF_comedor_horario_actualizado DEFAULT SYSUTCDATETIME(),
                    CONSTRAINT UQ_comedor_horario_codigo UNIQUE(codigo)
                );
            END;
            ...
            BEGIN
                ...
                EXEC sys.sp_executesql N'
                    ...
                    WHERE NOT EXISTS (SELECT 1 FROM comedor.horario_operacion o
                                      WHERE o.id_horario_origen=h.IdHorario);';
            END;
            IF OBJECT_ID(N'dbo.Horario', N'U') IS NOT NULL
               AND COL_LENGTH(N'dbo.Horario', N'HoraLimite') IS NULL
            ...
            IF NOT EXISTS (SELECT 1 FROM comedor.horario_operacion WHERE codigo='nocturno')
                INSERT comedor.horario_operacion(codigo,descripcion,hora_limite,origen)
                VALUES ('nocturno',N'Nocturno','20:00','valor_predeterminado');
            """
        )
    )
...
def downgrade() -> None:
    raise RuntimeError("La configuración de horarios del comedor no admite reversión destructiva")
