...
def main():
    ...
    with Session(motor) as sesion:
        ...
        sesion.add(persona)
        sesion.flush()
        sesion.add(CuentaAdministrativa(persona_id=persona.id, usuario=usuario,
            contrasena_hash=hash_secreto(contrasena), rol="administrador",
            activo=True, vinculacion_pendiente=False))
        sesion.commit()
    entorno = {**os.environ, "DATABASE_URL": URL, "CORS_ORIGIN": "https://localhost:55441",
        "APP_ENV": "test", "COOKIE_SECURE": "true", "CSRF_SECRET": clave,
        "CARNET_QR_CLAVE": clave, "IMPORTACION_RESULTADOS_KEY": clave,
        "STUDENT_MAX_LOGIN_ATTEMPTS": "8", "STUDENT_LOCK_MINUTES": "5",
        "ADMIN_MAX_LOGIN_ATTEMPTS": "5", "ADMIN_LOCK_MINUTES": "15",
        "STUDENT_SESSION_DAYS": "365", "ADMIN_SESSION_MINUTES": "60",
    ...
    with tempfile.TemporaryDirectory(prefix="scb-f4-tls-") as carpeta:
        ...
        api = subprocess.Popen([sys.executable, "-m", "uvicorn", "aplicacion.entrada:crear_aplicacion",
            "--factory", "--host", "127.0.0.1", "--port", "55440", "--no-access-log"],
        ...
        try:
            ...
            docker("run", "-d", "--name", "scb-f4-nginx-tls", "--network", "host",
                   "--mount", f"type=bind,src={config},dst=/etc/nginx/nginx.conf,readonly",
                   "--mount", f"type=bind,src={carpeta},dst=/certificados,readonly",
                   "nginx:alpine")
            nginx = True
            time.sleep(1)
            contexto = ssl.create_default_context(cafile=str(cert))
            with httpx.Client(base_url="https://localhost:55441/api/v1", verify=contexto) as cliente:
                login = []
                comedor = []
                for _ in range(20):
                    assert cliente.get("/autenticacion/csrf").status_code == 204
                    inicio = time.perf_counter()
                    respuesta = cliente.post("/autenticacion/administracion", json={
                        "usuario": usuario, "contrasena": contrasena}, headers={
                ...
                print(json.dumps({"transporte": "Nginx HTTPS, certificado verificado",
                    "muestras": 20, "login_p95_ms": round(sorted(login)[18], 2),
                    "comedor_p95_ms": round(sorted(comedor)[18], 2)}, indent=2))
        finally:
            if nginx:
