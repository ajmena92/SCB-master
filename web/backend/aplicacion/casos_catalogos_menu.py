...
class CasosCatalogosMenu:
    ...
    def listar_plantillas(self):
        return [
            ...
            for plantilla, componentes in self.repo.listar_plantillas()
        ]
    ...
    def guardar_plantilla(self, datos):
        ...
        componentes = [
            ComponenteMenu(**componente.model_dump()) for componente in datos.componentes
        ]
    ...
    def listar_calendario(self, desde, hasta):
        excepciones = {
            registro.fecha: registro for registro in self.repo.listar_calendario(desde, hasta)
        }
        sustituciones = {
            registro.fecha: registro for registro in self.repo.sustituciones_rango(desde, hasta)
        }
        ...
        while fecha <= hasta:
            ...
            origen, titulo, componentes = (
                ("cerrado" if es_lectivo and not habilitado else "no_lectivo"),
                None,
                [],
            )
            ...
            if habilitado and fecha in sustituciones:
                ...
                plantilla, componentes_menu = (
                    componentes_plantilla.get(posicion, (None, [])) if posicion else (None, [])
                )
    ...
    def guardar_sustitucion(self, datos):
        ...
        if len(ordenes) != len(set(ordenes)):
            raise HTTPException(422, "Los componentes no pueden repetir orden")
        componentes = [
            ComponenteSustitucionMenu(**componente.model_dump()) for componente in datos.componentes
        ]
