...
def crear_enrutador(
    obtener_repositorio: Callable[[], Iterator[RepositorioEstudiantes]],
    ...
    def servicio(
        repositorio: RepositorioEstudiantes = Depends(obtener_repositorio),
    ) -> ServicioEstudiantes:
    ...
    def listar(
        ...
        tamano: int = Query(25, ge=1, le=100),
        buscar: str = "",
        _u: dict = Depends(exigir_permiso("estudiantes.leer")),
        caso: ServicioEstudiantes = Depends(servicio),
    ) -> PaginaEstudiantes:
        return caso.listar(pagina, tamano, buscar)

    @enrutador.get(
        "/{id_estudiante}", response_model=EstudianteSalida, response_model_by_alias=True
    )
    def obtener(
        id_estudiante: int,
        _u: dict = Depends(exigir_permiso("estudiantes.leer")),
    ...
    def crear(
        ...
        usuario: dict[str, Any] = Depends(exigir_permiso("estudiantes.editar")),
        caso: ServicioEstudiantes = Depends(servicio),
    ) -> EstudianteSalida:
        try:
