...
class RepositorioSqlEstudiantes:
    ...
    def buscar_credencial_por_id(self, id_estudiante: int) -> dict | None:
        with self._fabrica.conexion() as conexion:
            cursor = conexion.cursor()
            cursor.execute("SELECT id_estudiante, carne, nombre, hash_contrasena, debe_cambiar_pin, fecha_expiracion_pin, activo FROM estudiantes.estudiante WHERE id_estudiante=?", id_estudiante)
            return self._fila(cursor)
    ...
    def buscar_por_id(self, id_estudiante: int) -> dict | None:
        with self._fabrica.conexion() as conexion:
            cursor = conexion.cursor()
            cursor.execute(
                """SELECT id_estudiante, carne, nombre, primer_apellido,
                segundo_apellido, cedula, seccion, activo FROM estudiantes.estudiante
                WHERE id_estudiante = ?""",
                id_estudiante,
            )
            return self._fila(cursor)
