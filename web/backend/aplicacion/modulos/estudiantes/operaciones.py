...
def crear_enrutador_operaciones(
    ...
    **_kwargs,
    ...
    def asistencia_hoy(
        ...
        marcas = [
            m
            for m in asistencia_repo.listar(date.today())
            if int(m["id_estudiante"]) == int(estudiante["idEstudiante"])
        ]
    ...
    def generar_pines_seccion(
        ...
        estudiantes = [
            e
            for e in repo.listar_para_generacion_pines()
            if (
                (datos.seccion is None and not e.get("seccion"))
                or e.get("seccion") == datos.seccion
            )
            and (not datos.turno or e.get("turno") == datos.turno)
        ]
        generados = [
            PinGenerado(
                idEstudiante=int(e["id_estudiante"]), pin=f"{secrets.randbelow(1_000_000):06d}"
            )
            for e in estudiantes
        ]
        repo.actualizar_pines_seccion(
            datos.seccion, {p.id_estudiante: hash_contrasena(p.pin) for p in generados}
        )
        filas = []
        for estudiante, pin in zip(estudiantes, generados):
            nombre = " ".join(
                str(estudiante.get(c, "") or "")
            ...
            filas.append(
                {
                    ...
                    "seccion": estudiante.get("seccion", datos.seccion),
                    "pin": pin.pin,
                }
            )
        if not filas:
