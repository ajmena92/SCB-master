...
class RepositorioBeneficios(Protocol):
    ...
    def asignar(self, id_estudiante: int, id_beneficio: int | None, id_usuario: int, ip: str) -> dict: ...
    ...
    def crear(self, datos: dict, id_usuario: int, ip: str) -> dict:
        with self._fabrica.conexion() as conexion:
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)""",
                datos["nombre"], datos.get("descripcion"), datos["dias_permitidos"],
                datos["activo"], id_usuario, ip or "WEB",
            )
    ...
    def actualizar(self, id_beneficio: int, datos: dict, id_usuario: int, ip: str) -> dict:
        with self._fabrica.conexion() as conexion:
            ...
            cursor.execute(
                ...
                WHERE id_beneficio=?""",
                datos["nombre"], datos.get("descripcion"), datos["dias_permitidos"],
                datos["activo"], id_usuario, ip or "WEB", id_beneficio,
            )
            if cursor.rowcount == 0:
                raise ValueError("Beneficio no encontrado")
            cursor.execute(
                """SELECT id_beneficio, nombre, descripcion, dias_permitidos, activo
                FROM beneficios.tipo_beneficio WHERE id_beneficio=?""", id_beneficio
            )
            resultado = self._fila(cursor)
            if resultado is None:
        ...
        with self._fabrica.conexion() as conexion:
            ...
            cursor.execute(
                ...
                WHEN NOT MATCHED THEN INSERT (id_estudiante, id_beneficio, creado_por, direccion_ip)
                    VALUES (?, ?, ?, ?);""",
                id_estudiante, id_beneficio, id_usuario, ip or "WEB",
                id_estudiante, id_beneficio, id_usuario, ip or "WEB",
            )
            cursor.execute(
