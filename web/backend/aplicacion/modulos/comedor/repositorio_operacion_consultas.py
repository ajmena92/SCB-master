...
class RepositorioSqlConsultasOperacion(RepositorioSqlComedorBase):
    def configuracion_operacion(self) -> dict:
        with self._fabrica.conexion() as conexion:
            ...
            cursor.execute("SELECT CAST(GETDATE() AS date),CONVERT(varchar(8),CAST(GETDATE() AS time),108)")
            ...
            horarios = [{"codigo": r[0], "descripcion": r[1], "hora_limite": r[2], "activo": bool(r[3])}
                        for r in cursor.fetchall()]
        return {"fecha_servidor": reloj[0], "hora_servidor": reloj[1],
                "minutos_aviso_previo": int(politica[0]),
                "permitir_marca_tardia": bool(politica[1]),
                "permitir_sin_marca_transporte": bool(politica[2]), "horarios": horarios}

    def historial_operacion(self, fecha: date, limite: int) -> list[dict]:
        with self._fabrica.conexion() as conexion:
            cursor = conexion.cursor()
            cursor.execute("""SELECT TOP (?) i.id_ingreso,i.id_persona,i.fecha,i.modalidad,
                ...
                WHERE i.fecha=? ORDER BY i.id_ingreso DESC""", limite, fecha)
