...
Public Class FrmDeploymentSetup
    ...
    Private ReadOnly _txtClientId As New TextBox()
    ...
    Private Sub InitializeLayout()
        ...
        AddHandler _txtServer.TextChanged, AddressOf OnLicenseRequestContextChanged
        AddHandler _txtDatabase.TextChanged, AddressOf OnLicenseRequestContextChanged
        ...
        AddField(licenseLayout, "Código cliente", _txtClientId)
        ...
        AddHandler _txtClientId.TextChanged, AddressOf OnLicenseRequestContextChanged
        ...
        AddField(licenseLayout, "Código de solicitud", _txtRequestCode)
    ...
    Private Sub LoadExistingState()
        ...
        If LicenseService.ValidateInstalledLicense(_txtServer.Text, _txtDatabase.Text, errorMessage, payload) AndAlso payload IsNot Nothing Then
            _hasValidLicense = True
            _txtCustomer.Text = payload.CustomerName
            _txtSite.Text = payload.SiteName
            _txtClientId.Text = payload.ClientId
            _txtActivationCode.Text = payload.ActivationCode
            _txtActivationCode.ReadOnly = True
            SetStatus("Licencia instalada: " & LicenseService.GetInstalledLicenseSummary(_txtServer.Text, _txtDatabase.Text), False)
        ElseIf LicenseService.TryLoadInstalledLicensePayload(payload, errorMessage) AndAlso payload IsNot Nothing Then
            ' En despliegue productivo puede existir la licencia antes de regenerar o restaurar deployment.config.json.
            _hasValidLicense = False
            _txtCustomer.Text = payload.CustomerName
            _txtSite.Text = payload.SiteName
            _txtClientId.Text = payload.ClientId
            _txtActivationCode.Text = payload.ActivationCode
            _txtActivationCode.ReadOnly = False
            SetStatus("Se detectó una licencia existente. Complete o valide la conexión SQL para reutilizarla.", True)
        Else
            _hasValidLicense = False
            _txtActivationCode.ReadOnly = False
            SetStatus("No existe licencia instalada. Debe validar conexión SQL y pegar un código de activación.", True)
        End If
    ...
    Private Sub OnSaveAndContinue(sender As Object, e As EventArgs)
        ...
        If Not _hasValidLicense Then
            If String.IsNullOrWhiteSpace(_txtCustomer.Text) OrElse String.IsNullOrWhiteSpace(_txtSite.Text) OrElse String.IsNullOrWhiteSpace(_txtClientId.Text) Then
                SetStatus("Debe indicar cliente, sede y código cliente.", True)
                Exit Sub
            End If

            If Not LicenseService.TryImportActivationCode(_txtCustomer.Text,
                                                          _txtSite.Text,
                                                          _txtClientId.Text,
                                                          _txtCustomer.Text,
                                                          config.Server,
                                                          config.Database,
                                                          _txtActivationCode.Text,
                                                          message) Then
                SetStatus(message, True)
                Exit Sub
            End If
    ...
    Private Sub RefreshRequestCode()
        _txtRequestCode.Text = LicenseService.BuildRequestCode(_txtCustomer.Text,
                                                               _txtSite.Text,
                                                               _txtClientId.Text,
                                                               _txtServer.Text,
                                                               _txtDatabase.Text)
