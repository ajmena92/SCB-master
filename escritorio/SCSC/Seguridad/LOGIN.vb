...
Partial Friend Class Login
    ...
    Private Const MaxManualConnectionRetries As Integer = 3
    ...
    Private BrandCrest As PictureBox
    ...
    Private Sub Login_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        If Not EnsureDeploymentReadyForLogin() Then
            AllowClose = True
        ...
        End If
        LoginStartTime = ServerClock.Now()
        ...
        BtnComedor.Image = CreateModuleIcon(True)
        BtnTransporte.Image = CreateModuleIcon(False)
    ...
    Private Function EnsureDeploymentReadyForLogin() As Boolean
        Dim readiness As DeploymentReadinessResult = DeploymentBootstrapper.EvaluateDeploymentReadiness()
        If readiness.IsReady Then
            Return True
        End If

        Dim manualRetryCount As Integer = 0

        Do While Not readiness.IsReady
            ErrorLogger.LogInfo(
                "Login.EnsureDeploymentReadyForLogin",
                "Fallo de conectividad/licencia. ReintentosManual=" & manualRetryCount.ToString() &
                ", ConfigOk=" & readiness.HasValidConfiguration.ToString() &
                ", LicenseOk=" & readiness.HasValidLicense.ToString() &
                ", Mensaje=" & readiness.FailureMessage)

            If DebeAbrirSetupDeInmediato(readiness) Then
                If Not AbrirSetupDespliegue(readiness) Then
                    Return False
                End If

                manualRetryCount = 0
                readiness = DeploymentBootstrapper.EvaluateDeploymentReadiness()
                Continue Do
            End If

            If manualRetryCount >= MaxManualConnectionRetries Then
                Dim openSetup As MsgBoxResult = MsgBox(
                    readiness.FailureMessage & vbCrLf & vbCrLf &
                    "Se agotaron los " & MaxManualConnectionRetries.ToString() & " intentos manuales de reconexión." & vbCrLf &
                    "¿Desea abrir la configuración de conexión ahora?",
                    MsgBoxStyle.YesNo Or MsgBoxStyle.Question,
                    "Configuración de conexión")

                ErrorLogger.LogInfo("Login.EnsureDeploymentReadyForLogin", "Respuesta abrir setup=" & openSetup.ToString())

                If openSetup <> MsgBoxResult.Yes Then
                    Return False
                End If

                If DeploymentBootstrapper.ShowDeploymentSetup(Me, False) <> DialogResult.OK Then
                    Return False
                End If
            ...
            Dim retryResult As MsgBoxResult = MsgBox(
                readiness.FailureMessage & vbCrLf & vbCrLf &
                "Intento manual de reconexión " & (manualRetryCount + 1).ToString() & " de " & MaxManualConnectionRetries.ToString() & "." & vbCrLf &
                "¿Desea reintentar ahora?",
                MsgBoxStyle.RetryCancel Or MsgBoxStyle.Exclamation,
                "Sin conexión a la base de datos")

            ErrorLogger.LogInfo("Login.EnsureDeploymentReadyForLogin", "Respuesta reintento=" & retryResult.ToString())
    ...
    Private Function DebeAbrirSetupDeInmediato(ByVal readiness As DeploymentReadinessResult) As Boolean
        If readiness Is Nothing OrElse readiness.IsReady Then
            Return False
        End If

        Dim profile As String = NormalizeConnectionProfile(GetConnectionProfile())
        If Not String.Equals(profile, DbProfileInstalled, StringComparison.OrdinalIgnoreCase) Then
            Return False
        End If

        If Not DeploymentConfigService.HasExternalConfiguration() Then
            Return True
        End If

        If Not LicenseService.HasInstalledLicenseFile() Then
            Return True
        End If

        Return readiness.HasValidConfiguration AndAlso Not readiness.HasValidLicense
    End Function

    Private Function AbrirSetupDespliegue(ByVal readiness As DeploymentReadinessResult) As Boolean
        Dim mensaje As String

        If Not DeploymentConfigService.HasExternalConfiguration() Then
            mensaje = "No existe deployment.config.json para este despliegue." & vbCrLf & vbCrLf &
                      "Se abrirá la configuración inicial para registrar conexión y licencia."
        ElseIf Not LicenseService.HasInstalledLicenseFile() Then
            mensaje = "No existe license.dat para este despliegue." & vbCrLf & vbCrLf &
                      "Se abrirá la configuración inicial para crear o importar la licencia."
        Else
            mensaje = readiness.FailureMessage & vbCrLf & vbCrLf &
                      "Se abrirá la configuración inicial para corregir la licencia actual."
        End If

        MsgBox(mensaje, MsgBoxStyle.Information, "Configuración inicial")
        Return DeploymentBootstrapper.ShowDeploymentSetup(Me, False) = DialogResult.OK
    End Function
    ...
    Private Function AutenticarUsuario() As Boolean
        ...
        Try
            ...
            If Not SeguridadService.ValidarContrasena(contrasenaIngresada, hashGuardado, saltGuardada) Then
                Dim fallo As SeguridadRbacService.LoginFailureStatus = SeguridadService.RegistrarLoginFallido(usuarioIngresado, CInt(usuario("IdUsuario")), "Credenciales inválidas.", "LOCAL")
                MsgBox(If(String.IsNullOrWhiteSpace(fallo.UserMessage), "Usuario o contraseña inválida.", fallo.UserMessage), MsgBoxStyle.Critical)
                Return False
            End If
    ...
    Private Sub BtnComedor_Click(sender As Object, e As EventArgs) Handles BtnComedor.Click
        If Not TryAuthorizeModuleAccess("comedor") Then
            Exit Sub
        End If

        Using frm As New ControlComedor()
            frm.ShowDialog()
        End Using
    End Sub

    Private Sub BtnTransporte_Click(sender As Object, e As EventArgs) Handles BtnTransporte.Click
        If Not TryAuthorizeModuleAccess("transporte") Then
            Exit Sub
        End If

        Using frm As New ControlTransporte()
            frm.ShowDialog()
        End Using
    End Sub

    Private Function TryAuthorizeModuleAccess(ByVal moduleKey As String) As Boolean
        If SeguridadPermisosSistema.IsModuleAccessibleWithoutLogin(moduleKey) Then
            Return True
        End If
        ...
        If String.IsNullOrWhiteSpace(NombreUsuario) Then
            NombreUsuario = usuarioIngreso
        End If

        EnsureRuntimeState()
        SeguridadService.EnsurePermissionCatalog()

        Dim accessContext As SeguridadRbacService.UserAccessContext = SeguridadService.GetUserAccessContext(CodigoUsuario)
        If accessContext Is Nothing OrElse Not accessContext.TienePermiso(SeguridadPermisosSistema.GetModulePermissionKeys(moduleKey)) Then
...
#End If
    ...
    Private Sub Login_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        ...
        If LoginStartTime <> DateTime.MinValue Then
            elapsedMs = CLng((ServerClock.Now() - LoginStartTime).TotalMilliseconds)
        End If
    ...
    Private Function CreateModuleIcon(ByVal isComedor As Boolean) As Image
        Dim bitmap As New Bitmap(72, 72)
        Using g As Graphics = Graphics.FromImage(bitmap)
            g.SmoothingMode = SmoothingMode.AntiAlias
            g.Clear(Color.Transparent)

            Using background As New SolidBrush(If(isComedor, Color.FromArgb(225, 244, 236), Color.FromArgb(225, 237, 251)))
                g.FillEllipse(background, 2, 2, 68, 68)
            End Using

            Dim tint As Color = If(isComedor, Color.FromArgb(20, 122, 82), Color.FromArgb(24, 100, 188))
            Using pen As New Pen(tint, 4.0F)
                pen.StartCap = LineCap.Round
                pen.EndCap = LineCap.Round
                If isComedor Then
                    g.DrawArc(pen, 18, 25, 36, 24, 0, 180)
                    g.DrawLine(pen, 18, 37, 54, 37)
                    g.DrawLine(pen, 25, 45, 47, 45)
                    g.DrawLine(pen, 29, 14, 29, 23)
                    g.DrawLine(pen, 24, 14, 24, 19)
                    g.DrawLine(pen, 34, 14, 34, 19)
                Else
                    g.DrawRectangle(pen, 16, 20, 40, 29)
                    g.DrawLine(pen, 16, 34, 56, 34)
                    g.DrawLine(pen, 24, 20, 24, 34)
                    g.DrawLine(pen, 48, 20, 48, 34)
                    Using wheelBrush As New SolidBrush(tint)
                        g.FillEllipse(wheelBrush, 21, 46, 8, 8)
                        g.FillEllipse(wheelBrush, 43, 46, 8, 8)
                    End Using
                End If
            End Using
        End Using
    ...
    Private Sub ApplyInstitutionalBranding()
        Me.Font = BrandTypography.CreateFont(10.0!, FontStyle.Regular)
        Label1.Font = BrandTypography.CreateFont(34.0!, FontStyle.Bold)
        Label3.Font = BrandTypography.CreateFont(11.5!, FontStyle.Regular)
        LblUsuarioCaption.Font = BrandTypography.CreateFont(10.0!, FontStyle.Bold)
        LblClaveCaption.Font = BrandTypography.CreateFont(10.0!, FontStyle.Bold)
        CodUsuario.Font = BrandTypography.CreateFont(13.0!, FontStyle.Regular)
        ClavePaso.Font = BrandTypography.CreateFont(13.0!, FontStyle.Regular)
        BtnLogin.Font = BrandTypography.CreateFont(11.5!, FontStyle.Bold)
        Label2.Font = BrandTypography.CreateFont(27.0!, FontStyle.Bold)
        LblInstitucion.Font = BrandTypography.CreateFont(11.0!, FontStyle.Bold)
        Version.Font = BrandTypography.CreateFont(8.5!, FontStyle.Regular)
        BtnComedor.Font = BrandTypography.CreateFont(12.0!, FontStyle.Bold)
        BtnTransporte.Font = BrandTypography.CreateFont(12.0!, FontStyle.Bold)

        If BrandCrest Is Nothing Then
            BrandCrest = New PictureBox()
            BrandCrest.Name = "BrandCrest"
            BrandCrest.Size = New Size(78, 78)
            BrandCrest.Location = New Point(390, 22)
            BrandCrest.Anchor = AnchorStyles.Top Or AnchorStyles.Right
            BrandCrest.BackColor = Color.Transparent
            BrandCrest.SizeMode = PictureBoxSizeMode.Zoom
            BrandCrest.TabStop = False
            Panel1.Controls.Add(BrandCrest)
            BrandCrest.BringToFront()
        End If

        Try
            Dim crestPath As String = ResolveBrandAssetPath("EscudoCtpPlatanares.png")
            If File.Exists(crestPath) Then
                Using source As Image = Image.FromFile(crestPath)
                    BrandCrest.Image = New Bitmap(source)
                End Using
            End If
        Catch ex As Exception
            ErrorLogger.LogException("Login.ApplyInstitutionalBranding", ex)
        End Try
    End Sub
