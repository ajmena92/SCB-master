...
Public Partial Class UIShellHost
    Public Sub SetDashboardLastUpdate(ByVal value As Date)
        If _lastUpdateBadge Is Nothing Then
            Exit Sub
        End If
        _lastUpdateBadge.Text = "Sync " & value.ToString("HH:mm:ss")
        If _navTooltip IsNot Nothing Then
            _navTooltip.SetToolTip(_lastUpdateBadge, "Ult. sync: " & value.ToString("yyyy/MM/dd HH:mm:ss"))
        End If
    End Sub
    ...
    Public Sub SetDashboardLastSuccessUpdate(ByVal value As Date)
        If _lastSuccessBadge Is Nothing Then
            Exit Sub
        End If
        _lastSuccessBadge.Text = "OK " & value.ToString("HH:mm:ss")
        If _navTooltip IsNot Nothing Then
            _navTooltip.SetToolTip(_lastSuccessBadge, "Ult. OK: " & value.ToString("yyyy/MM/dd HH:mm:ss"))
        End If
    End Sub
    ...
    Public Sub SetDashboardRefreshing(ByVal isRefreshing As Boolean)
        ...
        If isRefreshing AndAlso _refreshStatusBadge IsNot Nothing Then
            _refreshStatusBadge.Text = "Estado: sincronizando"
            _refreshStatusBadge.ForeColor = Color.FromArgb(28, 93, 158)
            If _navTooltip IsNot Nothing Then
                _navTooltip.SetToolTip(_refreshStatusBadge, "Sincronizando metricas y graficas del dashboard.")
            End If
        End If
    End Sub
    ...
    Public Sub SetDashboardRefreshResult(ByVal success As Boolean, ByVal message As String)
        ...
        If success Then
            _refreshStatusBadge.Text = "Estado: sincronizado"
            _refreshStatusBadge.ForeColor = Color.FromArgb(24, 121, 78)
        Else
            _refreshStatusBadge.Text = "Estado: error"
            _refreshStatusBadge.ForeColor = Color.FromArgb(173, 52, 61)
        End If
        If _navTooltip IsNot Nothing Then
            _navTooltip.SetToolTip(_refreshStatusBadge, _refreshStatusBadge.Text & " (" & safeMessage & ")")
        End If
    End Sub
    ...
    Public Sub SetDashboardNextRefreshText(ByVal text As String)
        If _nextRefreshBadge Is Nothing Then
            Exit Sub
        End If
        _nextRefreshBadge.Text = "Sig. " & If(String.IsNullOrWhiteSpace(text), "--", text.Trim())
        If _navTooltip IsNot Nothing Then
            _navTooltip.SetToolTip(_nextRefreshBadge, "Siguiente refresh: " & If(String.IsNullOrWhiteSpace(text), "--", text.Trim()))
        End If
    End Sub
    ...
    Public Sub SetDashboardRefreshQueueCount(ByVal count As Integer)
        ...
        _refreshQueueBadge.Text = "Pend. " & safeCount.ToString()
        If safeCount > 0 Then
        ...
        End If
        If _navTooltip IsNot Nothing Then
            _navTooltip.SetToolTip(_refreshQueueBadge, "Pendientes en cola: " & safeCount.ToString())
        End If
    End Sub
