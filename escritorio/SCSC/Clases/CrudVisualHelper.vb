...
Public NotInheritable Class CrudVisualHelper
    ...
    Public Shared Sub ApplyReportStandard(ByVal form As Form)
        ...
        ApplySurface(form)
        StyleButtons(form, "Imprimir")
        StyleDataGrids(form)
        NormalizeActionPanels(form, "Imprimir")
        EnsurePrimaryActionsVisible(form)
        ApplyResponsiveReportLayout(form)
    End Sub
    ...
    Private Shared Sub StyleButtons(ByVal root As Control, Optional ByVal primarySubmitText As String = "Guardar")
        For Each ctrl As Control In root.Controls
            If TypeOf ctrl Is Button Then
                Dim btn As Button = DirectCast(ctrl, Button)
                If IsPrimaryActionButtonName(btn.Name) Then
                    ApplyActionButtonStyle(btn, ResolveActionText(btn.Name, primarySubmitText))
            ...
            If ctrl.HasChildren Then
                StyleButtons(ctrl, primarySubmitText)
            End If
        Next
    End Sub
    ...
    Private Shared Sub NormalizeActionPanels(ByVal root As Control, Optional ByVal primarySubmitText As String = "Guardar")
        For Each ctrl As Control In root.Controls
            If TypeOf ctrl Is Panel Then
                ...
                If actionButtons.Count >= 2 Then
                    LayoutActionPanel(panel, actionButtons, primarySubmitText)
                End If
            End If

            If ctrl.HasChildren Then
                NormalizeActionPanels(ctrl, primarySubmitText)
            End If
        Next
    End Sub

    Private Shared Sub LayoutActionPanel(ByVal panel As Panel, ByVal buttons As List(Of Button), ByVal primarySubmitText As String)
    ...
    Private Shared Function ResolveActionText(ByVal name As String, ByVal primarySubmitText As String) As String
        Select Case name
            Case "BtnGuardar"
    ...
    Private Shared Sub ApplyModuleTitleStyle(ByVal form As Form)
        If form Is Nothing Then
            Exit Sub
        End If

        For Each titleName As String In New String() {"LblTituloModulo", "LblTitulo", "LabelTitulo"}
            Dim title As Label = TryCast(FindControlRecursive(form, titleName), Label)
            ...
            title.Left = Math.Max(16, title.Left)
            title.Top = Math.Max(12, title.Top)
            title.Width = Math.Max(320, form.ClientSize.Width - (title.Left * 2))
            title.Height = Math.Max(52, title.Height)
            title.Font = New Font("Segoe UI", 24.0!, FontStyle.Bold, GraphicsUnit.Point)
            title.ForeColor = UIConstants.TextPrimary
            title.TextAlign = ContentAlignment.MiddleLeft
        Next
    End Sub

    Private Shared Sub ApplyResponsiveReportLayout(ByVal form As Form)
        Dim title As Label = TryCast(FindControlRecursive(form, "LblTituloReporte"), Label)
        Dim parametersGroup As GroupBox = TryCast(FindControlRecursive(form, "GroupParametros"), GroupBox)
        Dim actionPanel As Panel = TryCast(FindControlRecursive(form, "PanelAcciones"), Panel)
        ...
        If title IsNot Nothing Then
            ...
            title.Left = 24
            title.Top = 20
            title.Width = Math.Max(360, form.ClientSize.Width - 48)
            title.Height = Math.Max(title.Height, 48)
            title.Font = New Font("Segoe UI", 24.0!, FontStyle.Bold, GraphicsUnit.Point)
            title.ForeColor = UIConstants.TextPrimary
        End If

        If actionPanel IsNot Nothing Then
            actionPanel.Anchor = AnchorStyles.Bottom Or AnchorStyles.Right
        End If

        If parametersGroup IsNot Nothing Then
            parametersGroup.Anchor = AnchorStyles.Top Or AnchorStyles.Bottom Or AnchorStyles.Left Or AnchorStyles.Right
            parametersGroup.Left = 24
            parametersGroup.Top = If(title Is Nothing, 24, title.Bottom + 16)
            parametersGroup.Width = Math.Max(520, form.ClientSize.Width - 48)

            Dim bottomReserve As Integer = If(actionPanel Is Nothing, 24, actionPanel.Height + 28)
            parametersGroup.Height = Math.Max(180, form.ClientSize.Height - parametersGroup.Top - bottomReserve)

            For Each ctrl As Control In parametersGroup.Controls
                If TypeOf ctrl Is ComboBox OrElse TypeOf ctrl Is DateTimePicker Then
                    If ctrl.Left > (parametersGroup.Width \ 2) Then
        ...
        If actionPanel IsNot Nothing Then
            actionPanel.Left = Math.Max(24, form.ClientSize.Width - actionPanel.Width - 24)
            actionPanel.Top = Math.Max(24, form.ClientSize.Height - actionPanel.Height - 18)
        End If
    End Sub
